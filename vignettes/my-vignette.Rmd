...
The **pr_summary** function the reads from an experimental block (\code{block_df}) and a reference list of fish present in this block (\code{block_ref_df}). It produces:
...
Why do we need both \code{block_df} and \code{block_ref_df}? If all the individuals have been read in \code{block_df}, then the individuals present in \code{block_ref_df} and \code{block_df} are the same. However, if some individuals haven't been read in \code{block_df} (e.g. animals haven't moved, lost the tag, are dead),
