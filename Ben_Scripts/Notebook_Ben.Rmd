...
Now, serious things start: we want to know how long individuals take to fully cross the box. To obtain this metric, I need to first define a few objects: my starting times (morning/afternoon).
...
The function below aims at, from one day of EB data, obtain, for each fish, three different metrics for exploration tendency. The function is built the following way:
...
ggplot(data = final, aes(time_since_start)) +
    geom_histogram(aes(),
                   bins = 20,
                   fill="#6f7b96",
                   colour = "white") +
...
```{r, message = FALSE}
    ...
    ggplot(data = final, aes(x = Treatment_seq, y = time_since_start, fill = time_of_day)) + 
    ...
    ggplot(data = final, aes(x = Treatment_seq, y = time_since_start)) + 
