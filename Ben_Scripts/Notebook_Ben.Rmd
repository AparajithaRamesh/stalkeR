...
Next, I want to keep the reads corresponding to antenna changes. For each individual, if an individual has been read at the antenna the following way (1, 1, 1, 2, 2, 3), we only keep the first reads for each sequence. The output is, for the example above: 1, 2, 3. 
...
First, as above, I want to keep the reads corresponding to antenna changes. For each individual, if an individual has been read at the antenna the following way (1, 1, 1, 2, 2, 3), we only keep the first reads for each sequence. The output is, for the example above: 1, 2, 3. 
...
```{r, message = FALSE}
missing_ind_df <- Reduce(intersect, list(EB_missing$missing_id,
...
week1 <- merge(EB_output[c("id", "Treatment_seq", "Pond", "time_since_start")],
      bottom_output[c("id", "Dist")],
      by = "id", all = T)

week1 <- merge(week1, 
      shelter_output[c("id", "duration_shelter")], by = "id", all = T)
