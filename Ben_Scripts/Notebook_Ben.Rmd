...
```{r, message = FALSE}
    ...
    ggplot(data = EB_Lat_final, aes(x = Treatment_seq, y = time_since_start, fill = time_of_day)) + 
      ...
      scale_y_continuous(expand = c(0, 0), limits = c(0,NA)) + ggtitle("Latency to explore")
...
shoaling.index <- function(clean_df, variables_df){
  #clean_df <- Top_clean_df
  #variables_df <- Top_variables
...
```{r}
    plot <- ggplot(data = Shoaling_tot, aes(x = Treatment_seq, y = SHOALING)) +
      geom_violin(fill = "#e4e3e8") + 
      stat_summary(fun.data=mean_sdl, 
                   fun.args = list(mult = 1), # I show 1 SD
                   geom = "pointrange", color="#414c61", 
