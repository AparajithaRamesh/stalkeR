...
There is no specific behavioural test for the last index. Instead, the shoaling metric is retrieved from the data collected in the four other tests. This notebook is so far written to compute the individuals behavioural indices for one week of treatment, from the four weekly data sets.
...
I import the four data sets we obtain every week, one for each behavioural test. I also import the list of all the fishes, with their corresponding pond and *treatment sequence* (_i.e._ PP, PM, MP, MM, where P stands for 'Plus predation treatment' and M for 'Minus predation treatment'.

```{r, message = FALSE}

# Exploration box data
EB_df <- read_delim("Data/Week 1/EB.CSV", 
     ";", escape_double = FALSE, trim_ws = TRUE)

# Bottom antennas data
bottom_df <- read_delim("Data/Week 1/bottom.CSV", 
     ";", escape_double = FALSE, trim_ws = TRUE)

# Top antennas data
top_df <- read_delim("Data/Week 1/top.CSV", 
     ";", escape_double = FALSE, trim_ws = TRUE)

# Shelters data
shelter_df <- read_delim("Data/Week 1/shelter.CSV", 
     ";", escape_double = FALSE, trim_ws = TRUE)
...
In this first section, you will find a series of generic functions, that can be applied to one of the data sets imported above (_i.e._ data frame containing the reads for one test during one day). These generic functions can be applied to data sets coming from any test.
...
The **clean.data** function does the following things:

- It subsets a few columns of interest from the raw data (_i.e._ time, antenna, id).
- It keeps only one read per individual and per second for a given antenna (_i.e._ remove repeats).
- It assigns every individual to a category named *time of the day*, indicating if the fish was tested during the morning or during the afternoon. The time of the days represent the two replicates for each treatment sequence.
...
- It removes the ghost reads, and the reads coming from antenna tests, only to keep the fish actually read in the ponds.
- It assigns each individual to its *pond number* and *treatment sequence*.

![Figure 1 - Location of the antennas for the *bottom* and the *shelter* tests.](C:/Users/benka/OneDrive/Documents/Cours/M2 - Sticklebacks/Meetings/Meeting 7 - Last practical details/Practicalities/Slide8.png){width=75%}

![Figure 2 - Location of the antennas for the *top* and the *EB* tests.](C:/Users/benka/OneDrive/Documents/Cours/M2 - Sticklebacks/Meetings/Meeting 7 - Last practical details/Practicalities/Slide9.png){width=75%}
...
# I remove all the reads that do not come from the fish present in the ponds 
# (e.g. ghost reads, test reads). I first identify them:
...
# I assign every individual to its pond and treatment category
    ...
    new_dataset <- new_dataset[order(as.POSIXct(new_dataset$time)),]
...
Next, I make a small function to obtain, for each clean data frame, the number of individuals in the pond that were read.

- Input = A clean data frame (see _clean.data_ function).
- Output = A data frame containing the number of individuals that were read per pond, and the total number.
...
Then, I write another small (very similar) function to obtain the identity of these missing individuals.
...
Next, I can apply these three generic functions to the different data sets (at the start of the next sections).

---

## 2. Latency to explore the box

An exploration box was introduced in the pond for 2:30 hours. Four different antennas are placed in the box, in a way that we can trace the movements of the fishes when they enter the box (see Figure 3.)

![Figure 3 - Exploration box](C:/Users/benka/OneDrive/Documents/Cours/M2 - Sticklebacks/Photos/Illustrations/Box.png){width=50%}
...
Next, I make a data frame with the characteristics of all individuals, even the ones that haven't been read this day. To do so, I use the _Fish list_ imported above. The main advantage of merging the clean data with the 'variables' data frame is to obtain the 'time of day' (i.e. if an individual was tested in the morning or afternoon), even for fish individuals that did not get read by the antennas.
...
### 2.2 Latency to explore the box

Now, we want to know how long individuals took to enter the exploration box. To obtain this metric, I need to first define two objects: my starting times (morning/afternoon).
...
Next, I obtain, for each individual, the first read at antenna 2 or 3 (i.e. first time inside the box). In a second step, I calculate the difference between this time and the time at which the box was introduced in the pond. 
...
## I obtain the first read at antenna 2 or 3
  index[[i]] <- subset(EB_list_df_id[[i]], antenna == 2 | antenna == 3)[1,]
...
# I remove the columns with NAs and rename my columns
  index <- na.omit(index)
  names(index)[2] <- "time_entering"


# I merge this table with the EB_variables data frame
  EB_output <- merge(index[c("id", "time_entering")], EB_variables, by = "id", all = T)

  

# I obtain the difference between the time of entrance, and the intial time for
# Morning
  EB_output_morning <- subset(EB_output, time_of_day == "Morning") %>%
    mutate(latency_entrance = as.numeric(difftime(time_entering, start_morning, units='m'))
           )

# Afternoon
  EB_output_afternoon <- subset(EB_output, time_of_day == "Afternoon") %>%
    mutate(latency_entrance = as.numeric(difftime(time_entering, start_afternoon, units='m'))
           )

# I join the two
  EB_output <- rbind(EB_output_morning, EB_output_afternoon)

# I give the maximal latency to the individuals that never entered the box
  EB_output <- EB_output %>%
    mutate(latency_entrance = if_else(is.na(latency_entrance), 150, latency_entrance))
...
The histogram below shows the distribution of latencies to enter the box. As you can see in the last chunk, fish that did not entirely cross the box received the maximal score.

```{r, message = FALSE}
  
ggplot(data = EB_output, aes(latency_entrance)) +
    geom_histogram(aes(),
                   bins = 20,
                   fill="#6f7b96",
                   colour = "white") +
    ...
    theme(axis.ticks.x = element_blank(),
          panel.background = element_rect(fill = "#f7f5f5")) + ggtitle("Latency to explore") + ylim(c(0,45))
...
```{r, message = FALSE}
    ...
    ggplot(data = EB_output, aes(x = Treatment_seq, y = latency_entrance, fill = time_of_day)) + 
      ...
      stat_summary(fun = median, geom="point", size=2, color="black", position = position_dodge(width = 0.4))


    ggplot(data = EB_output, aes(x = Treatment_seq, y = latency_entrance)) + 
      ...
      theme(panel.grid.major.y = element_line(colour = "#d4d4d4"),
            ...
            panel.background = element_blank(),
            axis.line.y = element_line(color = "black")) +
      labs(y = "Latency to enter the box (minutes)", x =" ") +
      scale_y_continuous(expand = c(0, 0), limits = c(0,NA)) + ggtitle("EB") + 
...
Here, we are interested in knowing how active are the fish, and how they move between antennas placed at the bottom of the ponds.
...
# Sequence of antenna changes corresponding to the distances above   
    ...
    seq.diag <- c('13', '31', '24', '42')
    seq.length  <- c('14', '41', '23', '32')
```
...
First, as above, I want to keep the reads corresponding to antenna changes. For each individual, if an individual has been read at the antenna the following way (1, 1, 1, 2, 2, 3), I only keep the first reads for each sequence of identical numvers. The output is, for the example above: 1, 2, 3. 
...
For every individual, indicate to what pond and treatment it is associated with.
...
- Input n°1 = A clean data frame containing all the reads (for a certain test) at one pond.
- Input n°2 = The variables of all the fish present in this pond.
...
- Input n°1 = A clean data frame containing all the reads (for a certain test) at *all* pond.
- Input n°2 = The variables of *all the fish*.
- Output = A data frame containing all the individuals (and not only the ones that were read), and for each of them, their number of accompanied reads, their total number of reads, and the accompanied reads divided by the total reads, (_i.e._ shoaling index).
...
I apply the function to the cleaned data sets, and assemble them as ggplot-friendly vertical table.
...
```{r}
  ...
  shoaling_bottom$test <- "bottom"
  
  shoaling_top <- shoaling.index(top_clean_df, top_variables)
  shoaling_top$test <- "top"
  
  shoaling_shelters <- shoaling.index(shelter_clean_df, shelter_variables)
  shoaling_shelters$test <- "shelter"
  
  shoaling_vertical <- rbind(shoaling_EB, shoaling_bottom,
                      shoaling_top, shoaling_shelters)
  

```


Then, I spread sum the reads (accompanied and total) together using _dplyr_, to give a global shoaling index (_i.e._ proportion of time spent with at least one conspecific, across tests).

```{r}
  
  # I depart from the shoaling_vertical df
  shoaling_output <- shoaling_vertical %>% 
  # I group by individuals
  group_by(id) %>% 
  # I create a new column with the sum of total and acc reads  
  mutate(tot_reads = sum(tot_reads), tot_acc_reads = sum(tot_acc_reads)) %>% 
  # I remove repeats  
  distinct(id, .keep_all = TRUE)

  # I make the shoaling index columns based on the number of reads across tests
  shoaling_output$shoaling <- shoaling_output$tot_acc_reads/shoaling_output$tot_reads
...
ggplot(data = shoaling_vertical, aes(shoaling)) +
    geom_histogram(aes(),
                   bins = 20,
                   fill="#6f7b96",
                   colour = "white") +
...
```{r}
 ...
 # Violin plots for differences between the 8 ponds within a week
ggplot(data = shoaling_output, aes(x = Treatment_seq, y = shoaling, fill = time_of_day)) + 
      ...
      labs(y = "Time spent hidin (minutes)", x =" ") +
      scale_y_continuous(expand = c(0, 0), limits = c(0,NA)) + ggtitle("Hiding") + 
      stat_summary(fun = median, geom="point", size=2, color="black", position = position_dodge(width = 0.5))
...
week1_df <- tibble(EB_output[c("id", "Treatment_seq", "Pond", "latency_entrance")],
