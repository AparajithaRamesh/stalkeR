...
# Shelters data
df_shelter <- read_delim("Data/Week 1/20201104.CSV", 
     ";", escape_double = FALSE, trim_ws = TRUE)
...
Next, I make a small function to obtain, for each clean data frame, then umber of individuals in the pond.

```{r}

Nb.ind <- function(clean_df){
# I separate my dataset per Pond
...
Here is an alternative version of the chunk above. It is less elegant, yet more understandible.
First, I build a function which has, as:

- Input = A dataframe containing the antenna change reads for one individual
...
# I add a new column with the time to cross the box since the start of the test
# For the morning
  EB_Lat_2_morning <- mutate(subset(EB_Lat_2, time_of_day == "Morning"), 
                           time_since_start = difftime(Ant2, start_morning, units='m'))
# For the afternoon
  EB_Lat_2_afternoon <- mutate(subset(EB_Lat_2, time_of_day == "Afternoon"), 
...
# I run the finalise.EB function defined above to clean my output
  EB_Lat_final_2 <- finalise.EB(EB_Lat_2, EB_variables)
...
I first run the **clean.data** function on the Shelter data frame, then split it per individual (same as above). I also look at my number of individuals per pond. 

```{r, message = FALSE}
  Shelter_clean_df <- clean.data(df_shelter)
