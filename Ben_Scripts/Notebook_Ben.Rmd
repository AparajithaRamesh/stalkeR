...
Below, you will find a series of generic functions, that can be applied to one of the data sets imported above (_i.e._ data frame containing the reads for one test during one day). These functions can be applied to data sets coming from the four different tests.
...
```{r, message = FALSE}
  ...
  # I bind the rows of the list (i.e. make it a data frame, as it initially was)
    ...
    EB_Changes <- merge(EB_variables[c("id", "Pond", "Treatment_seq", "time_of_day")], 
    EB_Changes, by = "id")
    EB_Changes <- EB_Changes[order(as.POSIXct(EB_Changes$time)),]
    rownames(EB_Changes) <- NULL
...
for (i in 1:length(EB_Changes_list)){
  ...
  EB_Changes_list[[i]] <- EB_Changes_list[[i]][order(as.POSIXct(EB_Changes_list[[i]]$time)),]

  # I mark all the row numbers at which the fish was read at antenna 1 or a 4
  vec <- which(EB_Changes_list[[i]][,"antenna"] == 1 | EB_Changes_list[[i]][,"antenna"] == 4)
  # I want to cut *after* antenna 1 and 4 were read
  vec <- vec  +1
  # I split my data frame every time the individual passes by ant 1 or 4
  list <- split(EB_Changes_list[[i]], cumsum(1:nrow(EB_Changes_list [[i]]) %in% vec))
  ...
  final[[i]] <- subset(EB_Changes_list[[i]], time == list[[1]]$time[1])
