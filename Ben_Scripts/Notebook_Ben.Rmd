...
This notebook contains chunks of R script computing incices for five different behaviours:
...
In this first section, you will find a series of generic functions, that can be applied to one of the data sets imported above (_i.e._ data frame containing the reads for one test during one day). These generic functions can be applied to data sets coming from the four different tests.
...
Next, I can apply these three generic functions to the different data sets (at the start of the next sections). The four following sections also contain chunks of code to specifically analyse the data from the four different tests, respectively. 
...
Next, I make a data frame with the characteristics of all individuals, even the ones that haven't been read this day. To do so, I use the _Fish list_ imported above. The main advantage of this 'variables' data frame is that it contains the 'time of day', even for fish individuals that did not get read by the antennas.
...
Now, we want to know how long individuals take to enter the exploration box. To obtain this metric, I need to first define a few objects: my starting times (morning/afternoon).
...
Then, I obtain, for each individual, the first read at antenna 2 or 3. Then, I calculate the difference between this time and the time at which the box was introduced in the pond. 
