...
# I assign every individual to its pond and treatment category
    ...
    Shelter_nreads$duration_min <- Shelter_nreads$nb/60
...
```{r}
ggplot(data = Shelter_nreads, aes(duration_min)) +
    geom_histogram(aes(),
                   bins = 6,
                   fill="#6f7b96",
                   colour = "white") +
...
```{r}
 ...
 # Violin plots for differences between the 8 ponds within a week
ggplot(data = Shelter_nreads, aes(x = Treatment_seq, y = duration_min, fill = time_of_day)) + 
      geom_violin(position = position_dodge(width = 0.4)) +
    ...
    ggplot(data = Shelter_nreads, aes(x = Treatment_seq, y = duration_min)) +
      geom_violin(fill = "#b4d3db") + 
