...
Next, I can apply the function above to all individuals. I then add a column containing the latency to fully explore the box. 
...
I make a small function, to modify my data frame containing the crossing sequence of all individuals that crossed the box in a way that I have a data set handy for plotting. In this function, I add the individuals variables (i.e. pond, Time of the day, treatment sequence, hereafter _variables_). Finally, I give the maximal value (that is, 150 minutes) to all individuals that never crossed the box.
...
I use the finalise.EB function to clean up the output and to obtain a plottable dataframe.
...
# I assign every individual to its pond and treatment category
    Travelled.distance <- merge(Bottom_variables, Travelled.distance, by = "id", all = T)

```


```{r}
ggplot(data = Travelled.distance, aes(Dist)) +
    geom_histogram(aes(),
                   bins = 15,
                   fill="#6f7b96",
                   colour = "white") +
...
# I assign every individual to its pond and treatment category
    ...
    Shelter_nreads[is.na(Shelter_nreads)] <- 0    
    Shelter_nreads$duration_shelter <- Shelter_nreads$nb/60
...
```{r}
ggplot(data = Shelter_nreads, aes(duration_shelter)) +
    geom_histogram(aes(),
                   bins = 20,
                   fill="#6f7b96",
                   colour = "white") +
...
```{r}
 ...
 # Violin plots for differences between the 8 ponds within a week
ggplot(data = Shelter_nreads, aes(x = Treatment_seq, y = duration_shelter, fill = time_of_day)) + 
      geom_violin(position = position_dodge(width = 0.4)) +
    ...
    ggplot(data = Shelter_nreads, aes(x = Treatment_seq, y = duration_shelter)) +
      geom_violin(fill = "#e4e3e8") + 
      geom_point(position = position_jitter(width = 0.06), alpha = 0.2) +
      #stat_summary(fun.data=mean_sdl, 
      #             fun.args = list(mult = 1), # I show 1 SD
      #             geom = "pointrange", color="#414c61", 
      #             position = position_dodge(width = 0.4)) +
...
```{r, message = FALSE}
  ...
  missing_top <- miss.ind(Top_clean_df)
  print(missing_top)
...
```{r, message = FALSE}
Top_shedule <- data.frame(Pond = c(4, 2, 5, 6, 1, 3, 7, 8),
...
Top_nreads <- data.frame(id = ids, nb = nreads)
...
# I assign every individual to its pond and treatment category
    Top_nreads <- merge(Top_nreads, Top_variables, by = "id", all = T)
    Top_nreads[is.na(Top_nreads)] <- 0    
    Top_nreads$duration_top <- Top_nreads$nb/60
...
```{r}
ggplot(data = Top_nreads, aes(duration_top)) +
    geom_histogram(aes(),
                   bins = 15,
                   fill="#6f7b96",
                   colour = "white") +
...
```{r}
 ...
 # Violin plots for differences between the 8 ponds within a week
ggplot(data = Top_nreads, aes(x = Treatment_seq, y = duration_top, fill = time_of_day)) + 
      geom_violin(position = position_dodge(width = 0.4)) +
      ...
      theme(panel.grid.major.y = element_line(colour = "#d4d4d4"),
            ...
            panel.background = element_blank(),
            axis.line.y = element_line(color = "black")) +
      labs(y = "Time spent at the top", x =" ") +
      scale_y_continuous(expand = c(0, 0))

    ggplot(data = Top_nreads, aes(x = Treatment_seq, y = duration_top)) +
      geom_violin(fill = "#e4e3e8") + 
      geom_point(position = position_jitter(width = 0.06), alpha = 0.2) +
      #stat_summary(fun.data=mean_sdl, 
      #             fun.args = list(mult = 1), # I show 1 SD
...
```{r, message = FALSE}
missing_ind_df <- Reduce(intersect, list(missing_EB$missing_id,
                       missing_bottom$missing_id,
                       missing_shelter$missing_id,
                       missing_top$missing_id))
missing_ind_df <- data.frame(id = missing_ind_df)
missing_ind_df <- merge(missing_ind_df, list_fish[,c("id", "Pond")], by = "id", all = F)
...
week1 <- merge(EB_Lat_final[c("id", "Treatment_seq", "Pond", "time_since_start")],
      Travelled.distance[c("id", "Dist")],
      by = "id", all = T)

week1 <- merge(week1, 
      Shelter_nreads[c("id", "duration_shelter")], by = "id", all = T)


week1 <- merge(week1, 
      Top_nreads[c("id", "duration_top")], by = "id", all = T)
...
```{r, message = FALSE}
ggplot(data = week1, aes(x = duration_shelter, y = duration_top)) +
...
DO NOT READ BELOW. I REALISED ALL OF IT WAS INCORRECT. It will hopefully be fixed soon.
...
shoaling.index.pond <- function(clean_df_pond, variables_pond){
  

  # I split my data frame into a list of dataframes (one object per individual)
  # PROBLEM HERE, ANTENNA N°1 IN DIFF PONDS
  ...
  for (i in 1:8){
    ...
    variables_per_pond[[i]] <- subset(Bottom_variables, Pond == i)
    clean_df_list[[i]] <- subset(bottom_clean_df, Pond == i)
  ...
  hist(Shoaling_index$shoaling)
  

shoaling.index.pond(subset(bottom_clean_df, Pond == 8), subset(Bottom_variables, Pond == 8))
...
I write a function that computes the number of reads (i.e. ~ number of seconds, see *clean.data* function) each individuals spends accompanied, that is, read during the same second at the same antenna in presence of at least one conspecific. 

Input: A *cleaned* data frame containing all the reads for one day.
Output: A data frame containing all the fish that have been read in presence of a conspecific, and their corresponding: (i) number of accompanied reads, (ii) total number of reads, (iii) proportion of the time they spend in presence of a conspecific. The individuals that haven't been read in presence of a conspecific are not in the data frame.
...
```{r}
Shoaling_tot <- cbind(shoaling_EB[c("id", "Pond", "time_of_day", "Treatment_seq", "tot_reads", "tot_acc_reads")],
...
names(Shoaling_tot) <- c("id", "Pond", "time_of_day", "Treatment_seq", "tot_reads_EB", "acc_reads_EB",
                         "tot_reads_bottom", "acc_reads_bottom",
                         "tot_reads_top", "acc_reads_top",
                         "tot_reads_shelters", "acc_reads_shelters")

Shoaling_tot$ACC_READS <- Shoaling_tot$acc_reads_EB + Shoaling_tot$acc_reads_bottom + 
                              Shoaling_tot$acc_reads_top + Shoaling_tot$acc_reads_shelters

Shoaling_tot$TOT_READS <- Shoaling_tot$tot_reads_EB + Shoaling_tot$tot_reads_bottom + 
                              Shoaling_tot$tot_reads_top + Shoaling_tot$tot_reads_shelters


Shoaling_tot$SHOALING <- Shoaling_tot$ACC_READS/Shoaling_tot$TOT_READS
  
  
  
A <- ggplot(data = Shoaling_tot, aes(SHOALING)) +
    geom_histogram(aes(),
                   bins = 20,
                   fill="#6f7b96",
                   colour = "white") +
...
```{r}
    ggplot(data = Shoaling_tot, aes(x = Treatment_seq, y = SHOALING)) +
      geom_violin(fill = "#e4e3e8") + 
      stat_summary(fun.data=mean_sdl, 
                   fun.args = list(mult = 1), # I show 1 SD
                   geom = "pointrange", color="#414c61", 
